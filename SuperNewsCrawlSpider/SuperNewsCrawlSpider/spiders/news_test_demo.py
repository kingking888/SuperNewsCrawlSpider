...
class SupersinaNews(CrawlSpider):
    name = "news_test_demo"
    # allowed_domains = ['.*']
    start_urls = ["https://feed.mix.sina.com.cn/api/roll/get?pageid=153&lid=2509&k=&num=50&page={}"]
    gt = GetTime()
    header = header_util()
    page_count = 0
    ...
    def start_requests(self):
        for i in range(1,2):
            yield scrapy.Request(url=self.start_urls[0].format(i), callback=self.parse_item_home, headers=self.header)
    ...
    def parse(self, response):
        ...
        #     # title = Document.title(html_text)
        #     html = doc.summary()
